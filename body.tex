...
Vanilla environment definition are usually given for unit tests purposes or as an example.
...
\item operational activities, in stand alone mode, in an operational pipelineas part of a service. Some SW Products are libraries, therefore are not operationally, but used as dependencies by other software products.
\end{itemize}
...
All Git repositories composing the software product are dependencies in a GitHub \textit{metapackage} and released at the same time.
...
The software release is a monolithic snapshot of the software product to be used as is, a tag in the source code repository, or in its corresponding binary package, by the downstream processes or users.
...
It is created by building the software provided in the release tag. 
It contains also compiled library code, supporting scripts, documentation and other files provided but not compiled, required to run the software product.
...
Continuous integration is done on a ticket branch prior to merging changes to master. Binary packages (EUPS) and docker images for distribution are made available with the nightly and weekly builds (see next section \ref{sec:weekly}).
