...
Binaries packages should be generated only once and made available for their downstream use by dependent software or for deployment.
In some cases, the binary package may need to be rebuilt if a dependency change, without changing the released tag.
...
Therefore the version identification defined in later \ref{sec:versioning}, is not sufficient to identify the binary package. The build number shall be considered also.
